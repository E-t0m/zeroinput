...
**Stufenwechsel.** Wechselt die Stufe von 2 auf 1 (nach der vollständigen `multi_inverter_wait`-Hysterese), wird die Anforderung im selben Zyklus direkt auf die Stufe-1-Verteilung umgestellt; die einzelne verbleibende Stufe-1-Einheit übernimmt die gesamte Anforderung in einem Schritt. Da die Wechselrichter nach diesem harten Umschalten einige Sekunden zum Einschwingen brauchen und der Zähler die Umschalt-Transiente kurz sieht, wird `power_demand` für einige Zyklen nach dem Wechsel auf dem Wert vom Wechselzeitpunkt gehalten — die Verteilung übergeht den frisch berechneten Wert bewusst, damit der Regelkreis nicht gegen den eigenen Übergang arbeitet. Eine laufende Rampe oder ein Lastanstieg, der die Stufe zurück auf 2 hebt, verwirft das Halten sofort, sodass eine echte Laständerung unverzüglich bedient wird. Das Hochschalten (1→2) erfolgt sofort, ohne `multi_inverter_wait`-Verzögerung und ohne Halten. Während Stufe 2 hängt die verbose-/Web-Ausgabe `Nc` (N Zyklen) an die Einheiten-Zeile an — eine bedingte Schätzung, wie viele Zyklen noch bis zur Rückkehr auf Stufe 1 verbleiben, *sofern jeder zukünftige Wert auf oder unter dem Schwellwert bleibt*. Sie wächst wieder, sobald ein neuer hoher Wert in die Historie eintritt, da das Herunterschalten das gesamte Fenster zum Einschwingen braucht.
...
Kopfzeile und Datenzeilen richten sich nach der Spaltenliste `_MPPT_COLUMNS` (Label und Ausrichtung pro Spalte). Vor der Ausgabe werden alle Zeilen (Ladegeräte, `combined`, Temperatursensoren) gesammelt; `_mppt_render()` blendet danach jede Datenspalte aus, die über alle Zeilen hinweg leer ist — `device` und `name` bleiben immer sichtbar. Die `mode`-Spalte wird zusätzlich in der Breite an die tatsächlich vorkommenden Werte angepasst: schmal bei ausschließlich kurzen Werten wie `BULK`, breiter sobald `ABSORB`, `RECOND`, `EXTCON` oder `ErrorP` auftaucht. Die `Iload`-Spalte gilt zusätzlich als leer, wenn sie ausschließlich `OFF` oder leer ist, da das keine Lastinformation trägt. Welche Spalten sichtbar sind, wird bei jedem Aufruf neu bestimmt und folgt damit dem aktuellen Gerätezustand.

REC-Ausgabe über Hilfsfunktion `_drain_rec_msgs()` verzögert.
...
- `mode` — Victron: OFF/FAULT/BULK/ABSORB/FLOAT/EQUAL/START/RECOND/EXTCON; eSmart3: WAIT/MPPT/BULK/FLOAT/PRE; beide: `ErrorP` bei `CS='PORT'`
- Unkonfigurierte AGG-Geräte: `<SER#>  <Port-Name>  UNCONFIGURED` (eigenes Format, nach der Tabelle ausgegeben)
