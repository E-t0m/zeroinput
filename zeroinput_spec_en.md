...
`conf['alarms']['inverter_fault']: {cmd, interval}` — triggers in pure PV mode (`bat_discharge == 0`, `pv_power > 100W`, all inverters active) when `avg(long_send_history) > pv_power × n/(n-1) × 0.85`. A missing inverter causes zeroinput to ramp demand toward `pv_power × n/(n-1)` — this ratio is the detection signal. Fires verbose warning regardless; executes `cmd` at most every `interval` seconds.
...
Configured in the `alarms` conf block (separate from `rs485`), keyed by device name. Applies to eSmart3 devices (internal + external sensor) and AGG `temp_sensor` devices (external only).
...
The inverter-fault alarm is also stored in `conf['alarms']['inverter_fault']` (`cmd`, `interval`).
...
- **rs485 tab** — structured editor for RS485 port and device configuration; restart-required notice on save. PVp field hidden for `type: temp` devices.
- **alarms tab** — per-device temperature alarms (eSmart3: int\_hi/int\_lo/ext\_hi/ext\_lo; temp\_sensor: ext\_hi/ext\_lo) and inverter-fault alarm. Device labels use `temp_display` name when set. Cards collapsed on load.
