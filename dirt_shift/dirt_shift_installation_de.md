...
**Wallbox schaltet nicht wie erwartet.** Ein Handlauf mit `-v` zeigt die drei Pfade einzeln (`dirt_ok`, `energy_ok`, `voltage_ok`); eingeschaltet wird, sobald **einer** davon `True` ist, abgeschaltet erst, wenn **alle drei** `False` sind. Häufige Ursachen: `dirt_ok` bleibt `False`, obwohl beide `dirt%`-Werte unter ihren Schwellen liegen — dann fehlt die dritte Bedingung `mode free` (in derselben Zeile ablesbar). Oder `action: none` trotz passendem `should_on` — dann steht das Relais bereits im gewünschten Zustand. Wird gar nicht abgeschaltet, obwohl alle drei Pfade `False` sind, ist `marker(before)` `False`: `dirt_shift` schaltet grundsätzlich nur ab, was es selbst eingeschaltet hat.
