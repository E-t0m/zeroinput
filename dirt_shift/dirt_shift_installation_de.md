...
**`precharge_enabled` (Standard `false`, optional):** Aktiviert einen zusätzlichen Pfad, der gezielt PV-Überschuss in den Akku statt ins Haus lenkt (`pvpt` wird dort gedrosselt) — die einzige Ausnahme von der sonst uneingeschränkten `pvpt`-Garantie. Er ist das Spiegelbild der Entladelogik: Wie die Entladung in dreckige Stunden verschoben wird, wird das Laden in die **sehr sauberen** Stunden gezogen. Gedrosselt wird eine grüne Stunde, wenn sie zu den saubersten `precharge_cleanest_pct` % des Fensters gehört (Standard 25), mindestens 10 `dirt%`-Punkte sauberer ist als der kommende rote Block, **und** der erwartete Überschuss den freien Akkuraum (`battery_capacity_wh − Akkuinhalt`) übersteigt — also nur dann, wenn ohne Umlenken tatsächlich Überschuss verloren ginge. Mehrere sehr saubere Stunden drosseln gemeinsam; mit wachsendem Akkuinhalt läuft die Drosselung von selbst aus. Setzt `battery_capacity_wh` (nutzbare Kapazität in Wh) voraus. Details und die genaue Formel stehen in `dirt_shift_spec_de.md`, Abschnitt „Precharge".

**SMARD ist Voraussetzung.** `dirt_shift` bezieht reale Day-Ahead-Netzdaten (Bundesnetzagentur, kostenlos, ohne Anmeldung) für heute **und morgen** und leitet daraus die Zonen ab (Median-Schnitt über das rollierende 24-Stunden-Fenster: sauberere Hälfte grün, dreckigere rot); eine alternative Quelle für *echte* Netzdaten gibt es nicht (bei Ausfall greift ein modellierter Notmodus, siehe unten). Schlägt die Abfrage fehl, überbrückt der Cache genau **einen Tag**; danach schaltet `dirt_shift` in einen **Notmodus**, der die Zonen aus der Wetterprognose (Solar plus Wind über Open-Meteo) und einem festen Profil der bundesweiten Netzlast modelliert — deutlich gekennzeichnet, nie mit echten Daten verwechselt. Erst wenn auch das Wetter nicht abrufbar ist, hinterlässt `dirt_shift` einen „Alles-erlaubt"-Timer. Optional kann `vz_dirtiness_uuid` (eine vorher in volkszähler angelegte Kanal-UUID) gesetzt werden, damit `dirt_shift` den aktuellen Dreckigkeitswert bei jedem Lauf per HTTP-POST in volkszähler protokolliert (leer = deaktiviert).

Die Strahlungsprognose (Open-Meteo, `shortwave_radiation`, kostenlos, ohne Anmeldung) skaliert die empirische PV-Referenzkurve auf das tatsächliche Tageswetter (heute und morgen, kein API-Key nötig). Im selben Aufruf wird `wind_speed_100m` mitgeholt und gecacht, das nur der Notmodus braucht — so entsteht kein zusätzlicher Netzabruf.
...
`idle` bzw. `engaged` am Ende der Energie- und der Spannungszeile zeigt, welche der beiden Hysterese-Schwellen des jeweiligen Pfads gerade gilt — beide fordern zum Einschalten mehr als zum Halten; `marker(before)` ist der Besitzer-Merker vor diesem Lauf. `-debug` zeigt zusätzlich die stündliche Übersichtstabelle (PV-Referenzkurve, Strahlungsprognose, Klarhimmel-Index, erwartete PV, `basic_load`, Lade-/Entlade-Markierung, Dreckigkeit, Zone) sowie die geschriebenen Timer-Zeilen. Die Tabelle läuft **chronologisch ab der aktuellen Stunde** (erste Zeile) und über Mitternacht hinweg in den Folgetag. Sie ist also kein Kalendertag, sondern das rollierende 24-Stunden-Fenster, in dem alle Berechnungen vorwärts lesen — ab dem Sprung von `23` auf `0` sind die Zeilen Prognose für morgen. Mit `-avgnew` werden alle Caches (7-Tage-Mittel, PV-Kurve, Strahlungsprognose, SMARD) verworfen und neu abgefragt.
...
Fehlt der **Akkuenergieinhalt** (volkszähler-Lücke bei der Spannung), lässt sich die Batterie nicht steuern — sie wird freigegeben (Frei-Timer). Die **Wallbox-Steuerung läuft trotzdem weiter**: Ihre drei Pfade sind redundant, ein fehlender Wert lässt nur seinen eigenen Pfad entfallen (ohne Inhalt der Energie-Pfad, ohne Spannungsmessung der Spannungs-Pfad), die übrigen entscheiden normal. Nur wenn **alle** Wallbox-Pfade mangels Daten nicht entscheidbar sind, bleibt das Relais unverändert (kein Ein- oder Ausschalten). Ebenso bei fehlenden SMARD-Daten: Dort speist der Notmodus die Zonen, und der ganze Lauf inklusive Wallbox läuft normal.

Nur bei einem wirklich harten Fehler (volkszähler liefert keine vollständigen Tage für die Mittelung, oder weder SMARD noch das Wetter-Modell verfügbar) bricht `dirt_shift` ab, schreibt aber zuvor — sofern der Timer-Pfad bekannt ist — eine „Alles-erlaubt"-Zeile mit dem aktuellen Datum, damit zeroinput nicht durch eine veraltete Begrenzung blockiert wird; das Wallbox-Relais bleibt dabei unangetastet:
...
**Notmodus-Meldung „SMARD unavailable — MODELLED emergency zones".** Die SMARD-Abfrage schlägt fehl und der Cache ist älter als einen Tag; `dirt_shift` arbeitet ersatzweise mit aus dem Wetter modellierten Zonen. Das ist eine begründete Schätzung, keine echten Netzdaten — die Nachtzonen sollten nicht überbewertet werden. Sobald SMARD wieder liefert, greift automatisch der Normalbetrieb. Bricht `dirt_shift` ganz mit „SMARD zone data unavailable" ab, ist zusätzlich das Wetter nicht abrufbar; dann läuft zeroinput über den „Alles-erlaubt"-Timer uneingeschränkt weiter. Netzwerkverbindung, SMARD- und Open-Meteo-Erreichbarkeit prüfen.
...
**Warnung „battery content came out … below empty".** Der rekonstruierte Energieinhalt ist deutlich negativ geworden (unter −5 % der `battery_capacity_wh`), was physikalisch unmöglich ist: Seit dem letzten Leerzustand wurde mehr Entladung als Ladung gezählt. Die Meldung rechnet dieselbe Kanalbilanz zusätzlich **ganz ohne** jeden Wirkungsgrad und unterscheidet danach zwei Fälle:

- **„Efficiency settings cannot explain this … check the inverters."** Selbst ohne jeden angenommenen Umwandlungsverlust bleibt die Bilanz negativ — das kann keine Wirkungsgrad-Einstellung erklären, da Wirkungsgrade den Inhalt nur nach unten drücken können, nie nach oben. Die Kanaldaten selbst sind dann falsch: typischerweise ein Wechselrichter, der Leistung *anfordert*, aber nicht *liefert* — nach einem ausgelösten FI/RCD, einer durchgebrannten Sicherung oder einem defekten Gerät loggt der Inverter-Kanal eine Entladung, die real nie floss.
- **„… PV_to_bat_efficiency / bat_to_AC_efficiency may be set lower than this installation's real losses …"** Ohne Wirkungsgrad wäre die Bilanz plausibel — die konfigurierten Prozentsätze könnten schlicht zu niedrig (zu pessimistisch) für die reale Anlage sein. Gegen einen echten Leer-zu-Leer-Zyklus prüfen und ggf. anheben. Ein Wechselrichter-Fehler bleibt daneben möglich, die Meldung nennt ihn deshalb als Alternative.

In beiden Fällen macht zusätzlich ein zu weit zurückliegender Leer-Anker (kein echter Leerzustand seit vielen Tagen) den Effekt größer, weil sich der Fehler über eine längere Integration stärker aufsummiert. Die Regelung läuft derweil weiter (ein negativer Inhalt liegt ohnehin unter jeder Reserve → Batterie geschont); nach dem nächsten echten Leerzustand kalibriert sich der Inhalt von selbst.
