...
Zusätzlich muss der Entlademodus zulassen, dass der Akku dabei nichts verliert. Ein Auto muss ohnehin aus dem Netz geladen werden — es geht also nur darum, die sauberste Stunde dafür zu wählen und dabei die zwei Wege auszuschließen, auf denen eine Wallbox den Akku kosten kann:

| Modus | Wallbox | Begründung |
|---|---|---|
| `free` | erlaubt | Reserve gedeckt, oder die eine priorisierte dreckigste Stunde |
| `limit` | gesperrt | Der Akku darf noch bis Rate und Viertelstunden-Budget entladen, und dieses Budget ist für den Hausverbrauch gedacht, nicht für ein Auto |
| `stop` | erlaubt, **solange die Stunde keinen PV-Überschuss hat** | Entladung ist komplett gesperrt (`000`), die Wallbox kann dem Akku also kein einziges Wh entnehmen. Was sie sehr wohl könnte: den Überschuss abfangen, der den Akku Richtung Reserve auffüllen soll — genau das schließt die Überschussprüfung aus |

Die `limit`-Zeile entscheidet in der Praxis nie etwas: `limit` tritt nur in roten Stunden auf, dort liegt `dirt%` per Definition auf oder über dem Median, und schon der Median-Test oben schlägt fehl.

Für die Überschussprüfung zählt die **Stundenprognose**, nicht der Messwert (anders als beim Viertelstunden-Budget, siehe Ausgabe: timer.txt): Gefragt ist, ob diese *Stunde* eine ist, die den Akku lädt — eine stündliche Größe beantwortet das, ohne die Wallbox bei durchziehenden Wolken viertelstündlich ein- und auszuschalten.
