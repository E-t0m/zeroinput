...
- **rot, Inhalt < Reserve, laufende Stunde ist *nicht* die dreckigste im Fenster** → **gedeckelt** auf `limit_discharge_rate` (Watt, Konfiguration) **und gleichzeitig** auf ein Viertelstunden-Energiebudget von `¼ × (min(100 %, reserve_pct) × basic_load − erwartete PV)` dieser Stunde (siehe Rot-Reserve und dreckigste Stunde).
...
```
`wallbox_median_fraction` (Standard `30`, in Prozent) ist ein eigener, engerer Schnitt als der allgemeine Rot/Grün-Median — er passt sich der Streuung des jeweiligen Tages an. `wallbox_absolute_max` (Standard `50`) verhindert, dass an einem durchgehend dreckigen Tag (hoher Median) die relative Schwelle allein noch eine spürbar dreckige Stunde durchwinkt. Der Median wird über alle 24 Stunden des rollierenden Tages gebildet (siehe CO₂-Intensitätsprofil), nicht nur über die verbleibenden.
...
```
content − headroom  >  reserve_pct × upcoming_red_demand
...
```
Hier zählt **nicht** die reguläre `reserve` (die überall sonst in `dirt_shift` verwendet wird), sondern eine eigene, wallbox-spezifische Berechnung: `upcoming_red_demand` schätzt grob den Bedarf der **nächsten** zusammenhängenden roten Phase — anders als `red_window_demand` bricht der Scan dabei **nicht** an der ersten Überschussstunde ab, sondern sucht gezielt weiter bis zur nächsten roten Stunde. Der Grund: Solange `now` selbst innerhalb einer laufenden Überschuss-Phase liegt, zeigt die reguläre `reserve` `0` — nicht weil heute Abend nichts gebraucht wird, sondern weil das Modell davon ausgeht, der noch bevorstehende Überschuss werde den Akku von selbst füllen. Läuft die Wallbox aber genau in dieser vermeintlichen Überschussphase mit typischer Leistung, kann sie mehr ziehen, als der reale verbleibende Überschuss hergibt — der Akku würde dann netto entladen, während `reserve` weiterhin `0` anzeigt und die alte Energie-Bedingung dadurch praktisch wirkungslos wäre.
...
- **energie-Wh** — Energiebudget; im gedeckelten Modus `¼ × (min(100 %, reserve_pct) × basic_load − erwartete PV)` dieser Stunde, mindestens 0 (die tatsächliche Wh-Zuteilung für genau die laufende Viertelstunde), sonst `-1` = echt unbegrenzt (ein Sentinel, den `zeroinput.py`s `discharge_times.update()` erkennt und den Budget-Check dafür komplett überspringt), `000` = kein Budget (Stopp).

Die drei Modi sind also: `100 100 -1` (kein Limit), `<limit_discharge_rate> 100 <¼×(min(100 %,reserve_pct)×basic_load−PV)>` (gedeckelt), `000 100 000` (Stopp). Im gedeckelten Modus wirken zwei Deckel gleichzeitig und schließen sich gegenseitig die Lücke: `limit_discharge_rate` begrenzt nur die Momentanleistung — eine dauerhaft erhöhte, aber unterhalb dieser Schwelle bleibende Last liefe daran vorbei unbegrenzt weiter. Das Wh-Feld begrenzt dagegen die insgesamt entnommene Energiemenge dieser Viertelstunde auf `¼ × (min(100 %, reserve_pct) × basic_load − erwartete PV)` — `reserve_pct` (Standard 90 %) skaliert dabei denselben Anteil des Bedarfs, den auch die Reserve-Berechnung ansetzt, statt nicht-priorisierten Stunden den vollen Bedarf zu erlauben; die PV wird davon unabhängig in voller Höhe abgezogen, weil `pvpt` sie in dieser Stunde ohnehin schon direkt durchleitet und sie nicht zusätzlich aus dem Akku kommen muss. Eine Stunde lang genug (Watt) und hoch genug (Wh) zu ziehen, umgeht daher keinen der beiden Deckel. `limit_discharge_rate` ist eine anlagenspezifische Konfiguration; das Wh-Budget ist fest und braucht keinen eigenen Konfigurationseintrag, da es sich direkt aus `basic_load`, `reserve_pct` und der Strahlungsprognose ergibt.
...
- `wallbox_median_fraction` — relative Sauberkeits-Schwelle für die Wallbox in Prozent vom Tages-Median (Standard `30`).
...
Das Viertelstunden-Energiebudget (`¼ × (min(100 %, reserve_pct) × basic_load − erwartete PV)`, siehe Ausgabe: timer.txt) ist **keine eigene** Konfigurationsoption — es ergibt sich direkt aus `basic_load`, `reserve_pct` und der Strahlungsprognose und braucht deshalb keinen eigenen Wert in `dirt_shift.conf`.
