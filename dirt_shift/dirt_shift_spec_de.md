...
Die `-v`-Ausgabe zeigt die ermittelte dreckigste Stunde in der Zeile `red: content ... < reserve ... -> dirtiest hour HH:00`; in der `-debug`-Tabelle trägt dieselbe Stunde ein `!` an ihrer `chg`-Markierung (`D!`, gegenüber schlichtem `D` für jede andere Entlade-Stunde und `L` fürs Laden).
