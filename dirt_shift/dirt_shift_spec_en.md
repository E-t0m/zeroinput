...
- **red, content < reserve, running hour is *not* the dirtiest in the window** → **capped** to `limit_discharge_rate` (Watt, config) **and simultaneously** to a quarter-hour energy budget of `1/4 × (min(100 %, reserve_pct) × basic_load − expected PV)` for that hour (see Red reserve and the dirtiest hour).
...
If current content does not cover the reserve (`content < reserve`), every red hour is no longer served unrestricted by default. Instead, within the same window (now up to the next surplus hour), the **one** red hour with the highest `dirt%` is identified — ties go to the chronologically earliest one in the window. Only that hour stays unrestricted; every other red hour in the window is capped simultaneously to `limit_discharge_rate` (Watt) **and** to `1/4 × (min(100 %, reserve_pct) × basic_load − expected PV)` for that hour (Wh, quarter-hour energy budget) (see Discharge by zone).
...
The priority acts at exactly one point: which hour gets `free` instead of `limit`. It does not reserve any Wh explicitly for the dirtiest hour against the other red hours in the same window — instead, a non-priority red hour is capped, via the quarter-hour energy budget itself, to `min(100 %, reserve_pct) × basic_load − expected PV` per hour (four quarter-hours of `1/4` of that each). `reserve_pct` scales the same share of demand here as in the reserve calculation itself; expected PV is subtracted in full regardless, since it flows unrestricted via `pvpt` anyway and so needs no proportional "reserving". The additional `limit_discharge_rate` cap (Watt) only limits instantaneous power within the quarter-hour, not the total energy drawn.
...
- **energy-Wh** — energy budget; in capped mode `1/4 × (min(100 %, reserve_pct) × basic_load − expected PV)` for that hour, floored at 0 (the actual Wh allotment for exactly the running quarter-hour), otherwise `-1` = genuinely unlimited (a sentinel `zeroinput.py`'s `discharge_times.update()` recognises and skips the budget check for entirely), `000` = no budget (stop).

The three modes are thus: `100 100 -1` (no limit), `<limit_discharge_rate> 100 <1/4×(min(100 %,reserve_pct)×basic_load−PV)>` (capped), `000 100 000` (stop). Capped mode applies two caps at once, each closing the gap the other leaves open: `limit_discharge_rate` limits only instantaneous power — a load that stays permanently below this threshold but never lets up would otherwise run past it unrestricted. The Wh field instead limits the total energy drawn in that quarter-hour to `1/4 × (min(100 %, reserve_pct) × basic_load − expected PV)` — `reserve_pct` (default 90%) scales the same share of demand the reserve calculation itself assumes, rather than letting a non-priority hour draw its full demand; expected PV is subtracted in full regardless, since `pvpt` already passes it through directly during that hour and it need not also come from the battery. Drawing an hour's worth long enough (Watt) and high enough (Wh) at once therefore evades neither cap. `limit_discharge_rate` is an installation-specific setting; the Wh budget is fixed and needs no configuration entry of its own, since it follows directly from `basic_load`, `reserve_pct`, and the radiation forecast.
...
- `limit_discharge_rate` — Watt cap on discharge power during a non-priority red hour (default 3000); installation-specific, adapt to your own ordinary peak load. Applies together with the fixed quarter-hour energy budget (`1/4 × (min(100 %, reserve_pct) × basic_load − expected PV)`, floored at 0, not a config value of its own). Set very high, the power cap is effectively no longer in effect, leaving only the energy budget.
...
The quarter-hour energy budget (`1/4 × (min(100 %, reserve_pct) × basic_load − expected PV)`, see Output: timer.txt) is **not** a configuration option of its own — it follows directly from `basic_load`, `reserve_pct`, and the radiation forecast and therefore needs no value of its own in `dirt_shift.conf`.
